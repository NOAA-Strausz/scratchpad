...
years = list(range(2005,2025)) #adjust to decrease range
...
def load_and_standardize(shp_path):
    ...
    try:
        ...
        # --- 3. ENSURE ALL FIELDS EXIST (THE FIX) ---
        # Instead of just filling NaNs, we check if the column exists at all.
        # If not, we create it with a default empty string.
        
        # Make sure FIELDS_OF_INTEREST is accessible here (it is global in your script)
        all_required_cols = FIELDS_OF_INTEREST
        
        for col in all_required_cols:
            if col not in gdf.columns:
                # print(f"WARNING: Column {col} missing in {shp_path}. Creating empty column.")
                gdf[col] = "" 
            else:
                # Fill existing NaNs with empty strings
                gdf[col] = gdf[col].fillna("")

        # Also handle ICECODE specifically if not in FIELDS_OF_INTEREST logic
